...
import { men_shirt } from "../../../data/Men/men_shirt.js";
...
import { findProducts } from "../../../States/Products/Action.js";
...
export function ProductsPage() {
  ...
  const handelFilter = (value, sectionId) => {
    ...
    let filterValue = serchParams.getAll(sectionId);
    if (filterValue.length > 0 && filterValue[0].split(",").includes(value)) {
      filterValue = filterValue[0].split(",").filter((item) => item != value);
      if (filterValue.length === 0) {
        serchParams.delete(sectionId);
      }
    } else {
      filterValue.push(value);
    }
    if (filterValue.length > 0) {
      serchParams.set(sectionId, filterValue.join(","));
    }
    const quary = serchParams.toString();
    navigate({ search: `?${quary}` });
  };
  const handelRadioFilter = (e, sectionId) => {
    const serchParams = new URLSearchParams(location.search);
    serchParams.set(sectionId, e.target.value);
    const quary = serchParams.toString();
    navigate({ search: `?${quary}` });
  };
  ...
  const handleOptionSelect = (option) => {
    const newSortValue = option.query;
                                  ...
                                  {section.options.map((option, optionIdx) => (
                                    <>
                                      <FormControlLabel
                                        key={option.value}
                                        onChange={(e) =>
                                          handelRadioFilter(e, section.id)
                                        }
                                        value={option.value}
                                        control={<Radio />}
                                        label={option.label}
